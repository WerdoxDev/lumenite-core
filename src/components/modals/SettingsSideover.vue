                        ...
                        <div class="relative w-max">
                            <TransitionChild as="template" enter="ease-in-out duration-500" enter-from="opacity-0" enter-to="opacity-100" leave="ease-in-out duration-500" leave-from="opacity-100" leave-to="opacity-0">
                                <div class="absolute top-5 -right-12 flex bg-gray-800 rounded-lg p-1 hover:bg-gray-700">
                                    <button class="rounded-md text-white focus:outline-none hover:text-gray-300" @click="$emit('close')">
                                        <XIcon class="h-6 w-6" />
                                    </button>
                                </div>
                            </TransitionChild>
                                    ...
                                    <div class="absolute inset-0 px-4 sm:px-6">
                                        <div class="flex flex-col">
                                            <div v-for="menu in menus" :key="menu.id">
                                                ...
                                                <button v-else class="rounded-lg p-1 mb-1 ml-1 hover:bg-gray-200 focus:bg-gray-200" :class="menu.id === selected.id ? 'bg-gray-200' : ''" @click="$emit('select-menu', menu)">
