                ...
                <TransitionChild as="template" enter="ease-out duration-300" enter-from="opacity-0 translate-y-4 sm:translate-y-0 sm:scale-95" enter-to="opacity-100 translate-y-0 sm:scale-100" leave="ease-in duration-200" leave-from="opacity-100 translate-y-0 sm:scale-100" leave-to="opacity-0 translate-y-4 sm:translate-y-0 sm:scale-95">
                    <div class="inline-block align-bottom bg-white rounded-lg text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:align-middle sm:max-w-lg sm:w-full">
                        <div class="bg-white px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
                            <div class="sm:flex sm:items-start">
                                <div class="mx-auto flex-shrink-0 flex items-center justify-center h-12 w-12 rounded-full bg-red-100 sm:mx-0">
                                    <ExclamationIcon v-if="state.modal.type == ModalType.Error" class="h-8 w-8 text-red-600" aria-hidden="true" />
                                </div>
                        ...
                        <div class="bg-gray-50 px-4 py-3 sm:px-6 sm:flex sm:flex-row-reverse items-center">
                            ...
                            <p v-if="state.modal.code" class="text-sm text-gray-500 text-left w-full mt-3 sm:mt-0">Code: {{ state.modal.code }}</p>
                        </div>
                    </div>
                </TransitionChild>
            </div>
...
export default defineComponent({
    ...
    setup() {
        const state = reactive({
            modal: { isOpen: false, title: "", text: "", type: ModalType.Success, code: null } as ModalState,
        });

        function open(title: string, text: string, type: ModalType, code: string | null) {
            state.modal.title = title;
            state.modal.text = text;
            state.modal.type = type;
            state.modal.code = code;
